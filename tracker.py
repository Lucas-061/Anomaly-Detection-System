...
def bbox_center(bbox: tuple[int, int, int, int]) -> tuple[int, int]:    #计算人体检测框的中心点坐标
    x1, y1, x2, y2 = bbox
    return int((x1 + x2) / 2), int((y1 + y2) / 2)


@dataclass
class Track:    #保存一个被跟踪人员的状态，包括 ID、位置、轨迹、速度、围栏状态和外观特征
    track_id: int
    bbox: tuple[int, int, int, int]    #人体框
    confidence: float
    center: tuple[int, int]    #中心点
    last_center: tuple[int, int]
    last_seen: float
    created_at: float
    history: list[tuple[int, int]] = field(default_factory=list)    #运动轨迹
    ...
    climb_frames: int = 0
    run_frames: int = 0
    feature: np.ndarray | None = None    #外观特征，保持识别同一个


class CentroidTracker:    #目标跟踪器，用于为人员分配 ID，并判断连续帧中是否为同一个人
    ...
    def update(self, detections: list[Detection], frame=None) -> list[Track]:    #根据当前帧检测结果更新所有人员轨迹，返回当前跟踪目标列表
        now = time.time()
    ...
    def _create_track(self, detection: Detection, feature: np.ndarray | None, now: float) -> None:    #为新出现的人体目标创建新的 Track，并分配新的人员 ID
    ...
    def _update_track(self, track: Track, detection: Detection, feature: np.ndarray | None, now: float) -> None:    #更新已有人员的位置、速度、轨迹和颜色特征
    ...
    def _match_score(self, track: Track, detection: Detection, feature: np.ndarray | None) -> float | None:    #综合中心点距离、框大小和 HSV 特征匹配 ID·
        center = bbox_center(detection.bbox)
    ...
    def _size_difference(self, old_bbox: tuple[int, int, int, int], new_bbox: tuple[int, int, int, int]) -> float:    #计算两个检测框面积变化，用于辅助目标匹配
    ...
    def _feature_distance(self, old_feature: np.ndarray | None, new_feature: np.ndarray | None) -> float | None:    #计算两个人体颜色直方图特征的差异，相似度比较
    ...
    def _extract_feature(self, frame, bbox: tuple[int, int, int, int]) -> np.ndarray | None:    #从人体框区域提取 HSV 颜色直方图，用于同一人物定位
        if frame is None:
            return None
